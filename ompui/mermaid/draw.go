...
func drawSubgraphLabel(sg *subgraph, g graph) (*drawing, drawingCoord) {
	...
	for lineIdx, line := range sg.label.lines {
		...
		labelX := from.x + width/2 - stringWidth(line)/2
		if labelX < from.x+1 {
			labelX = from.x + 1
		}
		for _, char := range line {
			runeWidth := Max(runeWidth(char), 1)
			if labelX < to.x {
...
func wrapTextInColor(text, c, styleType string) string {
	// Upstream used gookit/color for "html" style. ompui injects ANSI via
	// Colorize after render; keep plain text here (cli path).
	_ = c
	_ = styleType
	return text
}
