...
type graphProperties struct {
	data             *orderedMap[string, []textEdge]
...
func addNode(node textNode, data *orderedMap[string, []textEdge], nodeSpecs map[string]graphNodeSpec) {
...
func setData(parent textNode, edge textEdge, data *orderedMap[string, []textEdge], nodeSpecs map[string]graphNodeSpec) {
...
func mermaidFileToMap(mermaid, styleType string) (*graphProperties, error) {
	...
	data := newOrderedMap[string, []textEdge]()
	...
	properties := graphProperties{
		...
		graphDirection:   "",
		styleType:        styleType,
		paddingX:         defaultPaddingBetweenX,
		paddingY:         defaultPaddingBetweenY,
		subgraphs:        []*textSubgraph{},
	}
	...
	for _, line := range lines {
		...
		if endRegex.MatchString(trimmedLine) {
			if len(subgraphStack) > 0 {
				_ = subgraphStack[len(subgraphStack)-1]
		...
		if err != nil {
			if hasUnparsedEdgeSyntax(line) {
				return &properties, fmt.Errorf("unsupported or malformed graph edge: %s", strings.TrimSpace(line))
			}
			node := parseNode(line)
...
func hasUnparsedEdgeSyntax(line string) bool {
	squareDepth, roundDepth, curlyDepth := 0, 0, 0
	inQuotes := false
	for i := 0; i < len(line); i++ {
		...
		case '}':
			if !inQuotes && curlyDepth > 0 {
				curlyDepth--
			}
		}
		if inQuotes || squareDepth > 0 || roundDepth > 0 || curlyDepth > 0 {
			continue
		}
		rest := line[i:]
