...
func calculateLayout(sd *SequenceDiagram, config *Config) *diagramLayout {
	...
	for i, p := range sd.Participants {
		w := stringWidth(p.Label) + boxPaddingLeftRight
		if w < minBoxWidth {
			w = minBoxWidth
		}
		widths[i] = w
...
func renderSequence(sd *SequenceDiagram, config *Config) (string, error) {
	...
	lines = append(lines, buildLine(sd.Participants, layout, func(i int) string {
		...
		labelLen := stringWidth(sd.Participants[i].Label)
		pad := (w - labelLen) / 2
...
func involvedParticipants(events []Event, layout *diagramLayout) (int, int) {
	...
	for _, ev := range events {
		...
		case EventNote:
			if ev.Note != nil {
				if ev.Note.From != nil {
					note(ev.Note.From.Index)
				}
				if ev.Note.To != nil {
					note(ev.Note.To.Index)
				}
			}
		}
	}
...
func renderNote(n *Note, layout *diagramLayout, chars BoxChars) []string {
	if n == nil || n.From == nil {
		return nil
	}
	text := n.Text
	...
	from := layout.participantCenters[n.From.Index]
	to := from
	if n.To != nil {
		to = layout.participantCenters[n.To.Index]
	}
	left, right := from, to
	if right < left {
	...
	ensure := func(base string, need int) []rune {
		r := []rune(base)
		if len(r) < need {
			pad := make([]rune, need-len(r))
			for i := range pad {
				pad[i] = ' '
			}
			r = append(r, pad...)
	...
	need := anchor + boxW + 2
	if need < layout.totalWidth+1 {
		need = layout.totalWidth + 1
	}

	top := ensure(buildLifeline(layout, chars), need)
	mid := ensure(buildLifeline(layout, chars), need)
	bot := ensure(buildLifeline(layout, chars), need)

	top[anchor] = chars.TopLeft
	for i := 1; i < boxW-1; i++ {
		top[anchor+i] = chars.Horizontal
	}
	top[anchor+boxW-1] = chars.TopRight

	mid[anchor] = chars.Vertical
	col := anchor + 1
	for _, r := range text {
		if col < anchor+boxW-1 {
			mid[col] = r
			col++
		}
	}
	mid[anchor+boxW-1] = chars.Vertical

	bot[anchor] = chars.BottomLeft
	for i := 1; i < boxW-1; i++ {
		bot[anchor+i] = chars.Horizontal
	}
	bot[anchor+boxW-1] = chars.BottomRight

	return []string{
		strings.TrimRight(string(top), " "),
		strings.TrimRight(string(mid), " "),
		strings.TrimRight(string(bot), " "),
	}
}
